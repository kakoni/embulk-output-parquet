...
/** Parquet output plugin implementing Embulk OutputPlugin. */
public class ParquetOutputPlugin implements OutputPlugin {
  ...
  protected static final ConfigMapper CONFIG_MAPPER =
      CONFIG_MAPPER_FACTORY.createConfigMapper();

  protected static final TaskMapper TASK_MAPPER =
      CONFIG_MAPPER_FACTORY.createTaskMapper();

  /**
   * Task configuration for Parquet output plugin.
   */
  ...
  public ConfigDiff transaction(ConfigSource config, Schema schema, int taskCount, 
                                Control control) {
    try (ClassLoaderSwap<? extends ParquetOutputPlugin> ignored = 
            new ClassLoaderSwap<>(this.getClass())) {
  ...
  public TransactionalPageOutput open(TaskSource taskSource, 
                                     org.embulk.spi.Schema embulkSchema, int taskIndex) {
    ...
    try {
      ...
      throw new RuntimeException("Unexpected error opening Parquet output for task " 
          + taskIndex, e);
    }
  }
